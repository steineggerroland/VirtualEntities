...
class InMemoryStorageTest(unittest.TestCase):
    ...
    def test_storing_consumption(self, datetime_mock):
        ...
        datetime_mock.now.return_value = datetime.now(tzlocal())
    ...
    def test_loading_consumption(self, datetime_mock: Mock):
        db = InMemoryTimeSeriesStorageStrategy()
        # when
        db.append_power_consumption(ConsumptionMeasurement(datetime.now(tzlocal()) - timedelta(days=1), 1), "entity01")
        db.append_power_consumption(ConsumptionMeasurement(datetime.now(tzlocal()) - timedelta(hours=1), 2), "entity01")
        db.append_power_consumption(ConsumptionMeasurement(datetime.now(tzlocal()) - timedelta(minutes=1), 3),
                                    "entity01")
        db.append_power_consumption(ConsumptionMeasurement(datetime.now(tzlocal()), 4), "entity01")
        # then
        # reset behavior of mock
        datetime_mock.now.return_value = datetime.now(tzlocal())
        datetime_mock.fromisoformat.side_effect = datetime.fromisoformat
    ...
    def test_just_keeps_ten_consumptions(self, datetime_mock: Mock):
        ...
        for watt in range(20):
            db.append_power_consumption(ConsumptionMeasurement(datetime.now(tzlocal()), watt), "entity01")
        # then
        # reset behavior of mock
        datetime_mock.now.return_value = datetime.now(tzlocal())
        datetime_mock.fromisoformat.side_effect = datetime.fromisoformat
    ...
    def test_only_returns_for_entity_climate(self):
        db = InMemoryTimeSeriesStorageStrategy()
        # when
        db.append_room_climate(TemperatureHumidityMeasurement(datetime.now(tzlocal()), 23.1, 14.12), "entity01")
        # then
    ...
    def test_getting_in_time_range_consumption(self, datetime_mock: Mock):
        db = InMemoryTimeSeriesStorageStrategy()
        # when
        db.append_room_climate(TemperatureHumidityMeasurement(datetime.now(tzlocal()) - timedelta(days=1), 21, 1),
                               "entity01")
        db.append_room_climate(TemperatureHumidityMeasurement(datetime.now(tzlocal()) - timedelta(hours=1), 22, 2),
                               "entity01")
        db.append_room_climate(TemperatureHumidityMeasurement(datetime.now(tzlocal()) - timedelta(minutes=1), 23, 3),
                               "entity01")
        db.append_room_climate(TemperatureHumidityMeasurement(datetime.now(tzlocal()), 24, 4), "entity01")
        # then
        # reset behavior of mock
        datetime_mock.now.return_value = datetime.now(tzlocal())
        datetime_mock.fromisoformat.side_effect = datetime.fromisoformat
    ...
    def test_just_keeps_ten_climates(self, datetime_mock: Mock):
        ...
        for index in range(20):
            db.append_room_climate(TemperatureHumidityMeasurement(datetime.now(tzlocal()), index, index), "entity01")
        # then
        # reset behavior of mock
        datetime_mock.now.return_value = datetime.now(tzlocal())
        datetime_mock.fromisoformat.side_effect = datetime.fromisoformat
