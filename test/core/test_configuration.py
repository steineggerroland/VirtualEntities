...
class ConfigurationTest(unittest.TestCase):
    def test_complete_config(self):
        ...
        self.assertEqual("Kitchen", config.entities[1].name)
        self.assertEqual("room", config.entities[1].type)
        self.assertEqual("kitchen/sensor/temperature", config.entities[1].sources.list[0].mqtt_topic)
        self.assertEqual("temperature", config.entities[1].sources.list[0].measures[0].type)
        self.assertEqual("$.update.temperature", config.entities[1].sources.list[0].measures[0].path)

        self.assertEqual("Bathroom", config.entities[2].name)
        self.assertEqual("room", config.entities[2].type)
        self.assertEqual(15, config.entities[2].temperature_thresholds.critical_lower)
        self.assertEqual(20, config.entities[2].temperature_thresholds.optimal.lower)
        self.assertEqual(22, config.entities[2].temperature_thresholds.optimal.upper)
        self.assertEqual(30, config.entities[2].temperature_thresholds.critical_upper)
        self.assertEqual(50, config.entities[2].humidity_thresholds.critical_lower)
        self.assertEqual(65, config.entities[2].humidity_thresholds.optimal.lower)
        self.assertEqual(75, config.entities[2].humidity_thresholds.optimal.upper)
        self.assertEqual(90, config.entities[2].humidity_thresholds.critical_upper)
        self.assertEqual("bath/sensor/temperature", config.entities[2].sources.list[0].mqtt_topic)
        self.assertEqual("temperature", config.entities[2].sources.list[0].measures[0].type)
        self.assertEqual("$.temperature", config.entities[2].sources.list[0].measures[0].path)
        self.assertEqual("humidity", config.entities[2].sources.list[0].measures[1].type)
        self.assertEqual("$.humidity", config.entities[2].sources.list[0].measures[1].path)

        self.assertEqual("Jane", config.entities[3].name)
        self.assertEqual("person", config.entities[3].type)
        self.assertEqual("calendar", config.entities[3].sources.list[0].application)
        self.assertEqual("jane private", config.entities[3].sources.list[0].name)
        self.assertEqual("calendar.jane.private", config.entities[3].sources.list[0].url)
        self.assertEqual("calendar-user", config.entities[3].sources.list[0].username)
        self.assertEqual("secret-calendar", config.entities[3].sources.list[0].password)
        self.assertEqual("*/16 * * * *", config.entities[3].sources.list[0].update_cron)
        self.assertEqual("ffffff", config.entities[3].sources.list[0].color_hex)
        # referenced calendar
        self.assertEqual("calendar", config.entities[3].sources.list[1].application)
        self.assertEqual("jane job", config.entities[3].sources.list[1].name)
        self.assertEqual("calendar.job/jane", config.entities[3].sources.list[1].url)
        self.assertEqual("jane", config.entities[3].sources.list[1].username)
        self.assertEqual("secret", config.entities[3].sources.list[1].password)
        self.assertEqual("0 0 * * * *", config.entities[3].sources.list[1].update_cron)
        self.assertEqual("f0f0f0", config.entities[3].sources.list[1].color_hex)
        # notifications
        self.assertEqual("persons/jane/appointments", config.entities[3].destinations.planned_notifications[0].mqtt_topic)
        self.assertEqual("*/15 * * * * 0", config.entities[3].destinations.planned_notifications[0].cron_expression)
        self.assertEqual("daily-appointments", config.entities[3].destinations.planned_notifications[0].subject)
